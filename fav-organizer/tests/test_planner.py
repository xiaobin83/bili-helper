"""Tests for build_plan — operation planning from LLM classifications.
...
- Edge cases: empty inputs, all items invalid, all items dupes.
"""
...
def test_folder_not_empty_when_some_items_remain():
    ...
    source = _make_folder(2, title="旧文件夹", attr=1)
    classifications = [_make_classification(item1, category="科技")]
...
def test_folder_empty_after_moves_and_deletes():
    ...
    source = _make_folder(2, title="旧文件夹", attr=1)
    classifications = [_make_classification(item_move, category="科技")]
...
def test_folders_to_create_list():
    ...
    assert plan.folders_to_create == ["知识", "科技"]
...
def test_empty_inputs():
    """No classifications, no invalids, no dupes → empty plan."""
    plan = build_plan(
        classifications=[],
        existing_folders=[],
        invalid_items=[],
...
def test_multiple_classifications_produce_correct_plan():
    """Integration: classifications + invalids + dupes."""
    item1 = _make_item(1, bvid="BV001")
    item2 = _make_item(2, bvid="BV002")
    item3 = _make_item(3, bvid="BV003")
    item_bad = _make_item(4, bvid="BVbad", attr=1)
    item_dup = _make_item(5, bvid="BVdup")

    default = _make_folder(1, title="默认收藏夹", attr=0)
    source_a = _make_folder(2, title="收藏A", attr=1)
    source_b = _make_folder(3, title="收藏B", attr=1)
    zone_tech = _make_folder(4, title="科技", attr=1)

    classifications = [
        _make_classification(item1, category="编程开发"),
        _make_classification(item2, category="科技"),
